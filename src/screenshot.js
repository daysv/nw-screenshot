...
clipLayout.onmousemove = function () {
  var args = arguments;
  if (this.RAFmousemove) {
    this.RAFmousemove.apply(null, args)
  }
}
...
function init(img) {
  ...
  clipLayout.RAFmousemove = function (e) {
    if (null === pos) {
      return;
    }
...
function finishEdit(left, top, width, height, saveimg) {
  ...
  tmp.width = width * window.devicePixelRatio;
  tmp.height = height * window.devicePixelRatio;
